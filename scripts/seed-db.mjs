...
const check=await fetch(`${url}/rest/v1/products?select=id&limit=500`,{headers:{apikey:key,Authorization:`Bearer ${key}`}});
if(!check.ok) throw new Error(await check.text());
const seeded=await check.json();
console.log(`Seeded ${rows.length} products; database now contains ${seeded.length} products`);
