...
export default function TopNavbar(){
  ...
  return <nav className="sticky top-0 bg-white z-20 border-b border-black/5">
    <div className="flex relative max-w-frame mx-auto items-center justify-between md:justify-start py-5 md:py-6 px-4 xl:px-0">
      ...
      <SearchBox/>
      <div className="flex items-center ml-2"><Link href="/shop" className="block md:hidden mr-3 p-1"><Image src="/icons/search-black.svg" height={22} width={22} alt={t("search")}/></Link><CartBtn/><Link href="/account" className="p-1 text-sm font-medium">{t("account")}</Link></div>
    </div>
  </nav>
}
