...
export async function GET() {
  ...
  try {
    const rows = await supabaseRequest<any[]>(adminPath("discount_rules", isDemoAdmin() ? "select=*&limit=1" : "select=*&id=eq.1&limit=1"));
    const r = rows?.[0];
    return NextResponse.json(r ? { id: r.id ?? 1, name: r.name, enabled: r.enabled, stockThreshold: r.stock_threshold, percentage: r.percentage } : defaultRule);
  } catch (error) {
...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    await supabaseRequest(adminPath("discount_rules", isDemoAdmin() ? "" : "id=eq.1"), { method: "PATCH", headers: { Prefer: "return=minimal" }, body: JSON.stringify(adminBody(row)) });
    const ps = await supabaseRequest<any[]>(adminPath("products", "select=id,stock,discount&limit=500"));
    for (const p of ps || []) {
      ...
      if (shouldAuto && !hasManualDiscount) {
        await supabaseRequest(adminPath("products", `id=eq.${p.id}`), {
          method: "PATCH",
