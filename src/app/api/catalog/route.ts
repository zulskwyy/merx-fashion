...
export async function GET(){
  ...
  try{await ensureProductsSeeded(); const rows=await supabaseRequest<any[]>("products?select=*&is_active=eq.true&order=id.asc&limit=500");return NextResponse.json(rows?.length?rows.map(normalize):products);}catch{return NextResponse.json(products);}
}
